using System;
using System.Collections.Generic;
using System.Text;

namespace UltimaTileEditor
{
    internal class DataFiles
    {
        /*public static readonly List<string> Ultima5Files = [ "CREATE", "DNG1", "DNG2", "DNG3", "END1", "END2", "ENDSC", "ITEMS",
            "MON0", "MON1", "MON2", "MON3", "MON4", "MON5", "MON6", "MON7", "STARTSC",
            "STORY1", "STORY2", "STORY3", "STORY4", "STORY5", "STORY6", "TEXT", "TILES", "ULTIMA" ];*/

        public static readonly List<string> Ultima5Files = [ "TILES" ];

        public static readonly List<string> Ultima5ImageFiles = [ "TILES" ];

        /*public static readonly List<string> Ultima4Files = [ "SHAPES", "CHARSET", "ABACUS", "ANIMATE", "GYPSY", "INSIDE", "OUTSIDE",
            "PORTAL", "TREE", "WAGON", "HONCOM", "SACHONOR", "SPIRHUM", "VALJUS", "TITLE",
            "START", "KEY7", "RUNE_0", "RUNE_1", "RUNE_2", "RUNE_3", "RUNE_4", "RUNE_5",
            "STONCRCL", "HONESTY", "COMPASSN", "VALOR", "JUSTICE", "SACRIFIC", "HONOR",
            "SPIRIT", "HUMILITY", "TRUTH", "LOVE", "COURAGE"];*/

        public static readonly List<string> Ultima4Files = ["SHAPES"];

        public static readonly List<string> Ultima4ImageFiles = ["SHAPES"];
        /*public static readonly List<string> Ultima4ImageFiles = ["SHAPES", "CHARSET", "ABACUS", "ANIMATE", "GYPSY", "INSIDE", "OUTSIDE",
            "PORTAL", "TREE", "WAGON", "HONCOM", "SACHONOR", "SPIRHUM", "VALJUS", "TITLE",
            "START", "KEY7", "RUNE_0", "RUNE_1", "RUNE_2", "RUNE_3", "RUNE_4", "RUNE_5",
            "STONCRCL", "HONESTY", "COMPASSN", "VALOR", "JUSTICE", "SACRIFIC", "HONOR",
            "SPIRIT", "HUMILITY", "TRUTH", "LOVE", "COURAGE"];*/

        public static readonly List<string> Ultima3Files = ["SHAPES"];

        public static readonly List<string> Ultima3ImageFiles = ["SHAPES"];

        public static readonly List<string> Ultima2Files = ["ULTIMAII"];

        public static readonly List<string> Ultima2ImageFiles = ["ULTIMAII"];

        public static readonly List<string> Ultima1Files = ["MOND", "TILES", "TOWN"];

        public static readonly List<string> Ultima1ImageFiles = ["MOND", "TILES", "TOWN"];
    }
}
